...
if [ "$INSTALLED_VERSION" != "$REQUIRED_VERSION" ]; then
    ...
    DOWNLOAD_DIR="/tmp/d_$PACKAGE"
    mkdir -p "$DOWNLOAD_DIR"
    ipk_files="podkop-v0.7.8-r1-all.ipk
...
cronTask="10 4 * * * sh <(wget --no-check-certificate -q -O - https://raw.githubusercontent.com/WebMaster/Routers_3000/refs/heads/main/007.sh) 2>&1 | tee /root/run.log"
str=$(grep -i "10 4 \* \* \* sh \<\(wget --no-check-certificate -q -O - https://raw.githubusercontent.com/WebMaster/Routers_3000/refs/heads/main/007.sh\) 2\>&1 \| tee /root/run.log" /etc/crontabs/root)
if [ -z "$str" ] 
then
    echo "Add cron task auto run script 007.sh"
    echo "$cronTask" >> /etc/crontabs/root
fi
